...
namespace EasyOpenXml3
{
    class PivotTableExample
    {
        public PivotTableExample() { }
        ...
        public void Run()
        {
            ...
            string executePath = Directory.GetParent(Directory.GetParent(Directory.GetParent(System.IO.Path.GetDirectoryName(Assembly.GetEntryAssembly().Location)).ToString()).ToString()).ToString();
            string sourcePath = Path.Combine(executePath, "PivotData.txt");
             ...
             * sheet 2 - Pivot Table
             * 
             * The program append raw data
             * Update range (Open Excel, Formula Tab, Name Manager)
             * Existing pivot table referencing the range, so it update an existing pivot table
             * */
            #region Example 1
            ...
            /****
             * Create from scratch
             * create new excel with 
             * raw data in sheet 1
             * pivot table in sheet 2
             * */
            #region Example 2
            row = 1;
            int maxColumn = 1;
             ...
             * 
             * the data fields means 
             * 
             * AddPivotAxis - Page      = pivot table filter
             * AddPivotAxis - Row       = pivot table row
             * AddPivotAxis - Column    = pivot table column
             * AddDataValue             = pivot table value (sum)
             * 
             * in pivot table
             * */
            #region Example 3
            using (OpenXmlMemoryStreamDocument streamDoc = new OpenXmlMemoryStreamDocument(
                SmlDocument.FromFileName(Path.Combine(executePath, "QuarterlyUnitSales.xlsx"))))
            {
                using (SpreadsheetDocument doc = streamDoc.GetSpreadsheetDocument())
                {
                    WorksheetPart pivot = WorksheetAccessor.AddWorksheet(doc, "Pivot");
